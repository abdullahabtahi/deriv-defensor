...
def render_genai_explanation(partner_data: dict, reason_codes: list, risk_score: float):
    ...
    api_status = "🟢 Claude API" if genai.client else "🟡 Template Mode"
    st.caption(f"Powered by {api_status}")
    
    # Generate explanation with visible loading
    with st.spinner("🧠 Analyzing partner behavior patterns..."):
    ...
    with st.expander("📧 Draft Retention Email (GenAI-Powered)", expanded=False):
        with st.spinner("✍️ Drafting personalized email..."):
        ...
        with col2:
            if st.button("🔄 Regenerate", key="regen_email"):
                st.rerun()
        with col3:
    ...
    st.divider()
    st.caption("""
    💡 **How this works:** This explanation synthesizes SHAP feature importance values, 
...
if __name__ == "__main__":
    # Test component
    st.set_page_config(page_title="GenAI Test", page_icon="🤖")
